...
{
	/// <summary>
	/// Worker that generates id whose identifying elements are influenced by AWS S3 keyspace partitioning.
	/// </summary>
	/// <remarks>
	/// IDs generated are generated by this worker with the most varying/random element at the end, meaning that the sequence somes after the timestamp, which comes after worker id and data centre id.
	/// </remarks>
	public class CaspianIdWorker
	{
		const int SequenceBits = 12;
		const int TimestampBits = 42;
		const int WorkerIdBits = 5;
		const int DatacenterIdBits = 5;
		const long MaxWorkerId = -1L ^ (-1L << WorkerIdBits);
		const long MaxDatacenterId = -1L ^ (-1L << DatacenterIdBits);

		public const int TimestampLeftShift = SequenceBits;
		private const int WorkerIdShift = SequenceBits + TimestampBits;
		private const int DatacenterIdShift = SequenceBits + TimestampBits + WorkerIdBits;
		...
		public long baseEpoch = 1356998400000L;

		public CaspianIdWorker(long workerId, long datacenterId, long baseEpoch, long sequence = 0L)
		{
			WorkerId = workerId;
		...
		public virtual long NextId()
		{
			...
			id = (DatacenterId << DatacenterIdShift) | (WorkerId << WorkerIdShift) |
					 ((timestamp - baseEpoch) << TimestampLeftShift) | sequence;
